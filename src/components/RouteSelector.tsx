...
export function RouteSelector({
  ...
  const validVessels = useMemo(() => {
    const exact = combinations
      .filter((x) => norm(x.origin) === norm(value.origin) && norm(x.destination) === norm(value.destination))
      .map((x) => x.vessel_type);
  ...
  useEffect(() => {
    ...
    const destinationValid = !!currentRoute?.destinations.some((d) => norm(d) === norm(value.destination));
    ...
    const nextDestination = destinationValid && currentRoute
      ? currentRoute.destinations.find((d) => norm(d) === norm(value.destination)) ?? currentRoute.destinations[0] ?? ''
      : fallbackRoute.destinations[0] ?? '';
    const nextVessel = vesselValid ? value.vessel : validVessels[0] ?? value.vessel;

    if (nextOrigin !== value.origin || nextDestination !== value.destination || nextVessel !== value.vessel) {
  ...
  const setField = (field: keyof RouteSelection, v: string | number) => {
    if (field === 'origin') {
      ...
      const firstVessel = combinations.find((x) => norm(x.origin) === norm(nextOrigin) && norm(x.destination) === norm(firstDest))?.vessel_type
        ?? vesselTypes[0] ?? '';
      onChange({ ...value, origin: nextOrigin, destination: firstDest, vessel: firstVessel });
    } else if (field === 'destination') {
      const nextDestination = String(v);
      const firstVessel = combinations.find((x) => norm(x.origin) === norm(value.origin) && norm(x.destination) === norm(nextDestination))?.vessel_type
        ?? validVessels[0] ?? '';
      onChange({ ...value, destination: nextDestination, vessel: firstVessel });
    } else if (field === 'vessel') {
      if (validVessels.some((x) => norm(x) === norm(String(v)))) onChange({ ...value, vessel: String(v) });
    } else {
