...
// SIH demo mode is intentionally independent of the Render backend.
...
export function StatusStrip({ status }: { status: SystemStatus }) {
  const items = [
    { icon: Server, label: 'API', state: status.api, text: 'DEMO' },
    { icon: Database, label: 'DATA', state: status.database, text: 'LOCAL' },
    { icon: Cpu, label: 'ML', state: status.ml, text: 'READY' },
  ];
