  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight text-white">What-If Simulator</h1>
              ...
              <div className="space-y-1.5">
                ...
                >
                  {(routes?.routes ?? []).flatMap((r) =>
                    r.destinations.map((d) => `${r.origin} → ${d}`),
                  ).map((rt) => <option key={rt} value={rt}>{rt}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                ...
                <select
                  className="select"
                  value={input.vessel}
                  onChange={(e) => setInput({ ...input, vessel: e.target.value })}
                >
                  {(routes?.vessel_types ?? []).map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex rounded-lg border border-white/[0.07] bg-ink-800/70 p-0.5">
                {[7, 30, 90].map((h) => (
                  <button
                    key={h}
                    onClick={() => setInput({ ...input, horizon: h })}
            ...
            {mutation.isPending ? (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {Array.from({ length: 4 }).map((_, i) => <SkeletonBlock key={i} className="h-[140px]" />)}
                </div>
              </motion.div>
            ) : mutation.isError ? (
              ...
              >
                <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-slate-400">
                  <span className="chip border-white/10 bg-white/[0.04] text-slate-300">{input.cargo_quantity.toLocaleString()} t</span>
                  <span className="chip border-white/10 bg-white/[0.04] text-slate-300">${input.current_freight_rate}/t</span>
                  <span className="chip border-white/10 bg-white/[0.04] text-slate-300">Fuel ${input.fuel_price}/t</span>
                  <span className="chip border-accent-500/30 bg-accent-500/10 text-accent-200">{input.horizon}D</span>
                </div>

                <CostComparison options={result.options} recommended={result.recommended} height={240} />
                <div className="mt-5">
              ...
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center gap-3 py-16 text-center text-slate-500">
                <Play className="h-6 w-6 opacity-40" />
                <p className="text-sm">Adjust the inputs and run a simulation to see projected costs.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
