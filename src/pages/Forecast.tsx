    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold tracking-tight text-white">Forecast Intelligence</h1>
      ...
      {q.isLoading ? (
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => <SkeletonBlock key={i} className="h-[92px]" />)}
        </div>
      ) : q.isError ? (
        <InlineError error={q.error} onRetry={() => q.refetch()} />
      ) : f ? (
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
          <StatCard icon={TrendingUp} label="Predicted Rate" value={fmtUsd(f.predicted_rate)} delay={0} />
          <StatCard icon={Layers} label="Lower Bound" value={fmtUsd(f.lower_bound)} delay={0.06} tone="text-success-400" />
          <StatCard icon={Layers} label="Upper Bound" value={fmtUsd(f.upper_bound)} delay={0.12} tone="text-danger-400" />
          <StatCard icon={Gauge} label="Confidence" value={`${f.confidence.toFixed(1)}%`} delay={0.18} tone="text-accent-300" />
      ...
      <div className="panel p-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">{sel.origin} → {sel.destination} · {sel.vessel} · {sel.horizon}D</p>
          </div>
      ...
      {f && (
        ...
        >
          ...
          <p className="text-sm leading-relaxed text-slate-400">
            {f.reason ?? `The ${f.model} model projects a ${f.expected_change_pct >= 0 ? 'rise' : 'decline'} of ${fmtPct(Math.abs(f.expected_change_pct))} over ${sel.horizon} days, with ${f.confidence.toFixed(1)}% confidence and a range of ${fmtUsd(f.lower_bound)} to ${fmtUsd(f.upper_bound)}.`}
          </p>
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Mini label="Current rate" value={fmtUsd(f.current_rate)} />
            <Mini label="Predicted" value={fmtUsd(f.predicted_rate)} />
            <Mini label="Change" value={fmtPct(f.expected_change_pct)} tone={changeTone(f.expected_change_pct)} />
            <Mini label="Confidence" value={`${f.confidence.toFixed(1)}%`} tone="text-success-400" />
          </div>
        </motion.div>
      )}
    </div>
...
function StatCard({ icon: Icon, label, value, delay, tone = 'text-white' }: { icon: typeof Brain; label: string; value: string; delay: number; tone?: string }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className={`mt-2 text-lg font-bold num ${tone}`}>{value}</div>
    </motion.div>
  );
}
