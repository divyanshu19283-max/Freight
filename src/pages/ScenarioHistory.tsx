...
export function ScenarioHistory() {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      out = out.filter((r) => r.route.toLowerCase().includes(q) || r.vessel.toLowerCase().includes(q) || r.action.toLowerCase().includes(q));
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold tracking-tight text-white">Scenario History</h1>
          {isDemo && <DemoBadge />}
        </div>
      ...
      <div className="panel flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
        ...
        <select value={actionFilter} onChange={(e) => setActionFilter(e.target.value)} className="select sm:w-48">
      ...
      <div className="panel overflow-hidden">
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-white/[0.06] text-left">
                ...
                <Th label="Route" k="route" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
                <Th label="Vessel" k="vessel" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
                <Th label="Action" k="action" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
                <Th label="Confidence" k="confidence" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} right />
                <Th label="Risk" k="risk" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
                <Th label="Savings" k="savings" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} right />
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={7} className="p-5"><SkeletonRows rows={6} cols={7} /></td></tr>
              ) : error ? (
                <tr><td colSpan={7} className="p-5"><InlineError onRetry={() => { recQ.refetch(); sceQ.refetch(); }} /></td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={7}><EmptyState label="No scenarios match your filters" /></td></tr>
              ) : (
                filtered.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3"><span className="chip border-white/10 bg-white/[0.04] text-slate-300">{r.vessel}</span></td>
                      <td className="px-4 py-3 font-semibold text-white">{r.action}</td>
                      <td className="px-4 py-3 text-right num text-slate-300">{r.confidence.toFixed(1)}%</td>
                      <td className="px-4 py-3">
                        <span className={`chip ${tone.bg} ${tone.border} border ${tone.text}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${tone.dot}`} /> {r.risk}
                        </span>
                      </td>
                      <td className={`px-4 py-3 text-right num font-semibold ${r.savings >= 0 ? 'text-success-400' : 'text-danger-400'}`}>
                        {r.savings >= 0 ? '+' : '-'}{fmtInr(Math.abs(r.savings))}
                      </td>
                    </motion.tr>
                  );
                })
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="flex items-center justify-between border-t border-white/[0.06] px-4 py-3 text-xs text-slate-500">
            <span className="flex items-center gap-1.5"><History className="h-3.5 w-3.5" /> {filtered.length} records</span>
