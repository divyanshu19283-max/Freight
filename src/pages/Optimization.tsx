...
export function Optimization({ onNavigate }: { onNavigate: (p: PageId) => void }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold tracking-tight text-white">Procurement Optimizer</h1>
          {isDemo && <DemoBadge />}
        </div>
        <p className="text-sm text-slate-400">Find the lowest risk-adjusted procurement strategy.</p>
      </div>
      ...
      {q.isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => <SkeletonBlock key={i} className="h-[88px]" />)}
        </div>
      ) : q.isError ? (
        ...
        <>
          ...
          <motion.div
            ...
            className="panel relative overflow-hidden p-5 ring-1 ring-success-500/30"
          >
            <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-success-500/10 blur-3xl" />
            <div className="relative flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-success-500/15 ring-1 ring-success-500/30">
                  <Trophy className="h-6 w-6 text-success-400" />
                </div>
                <div>
                  <div className="label-mono text-success-400/80">Recommended strategy</div>
                  <div className="text-xl font-bold text-white">{o.recommended}</div>
                </div>
              </div>
            ...
            {o.options.map((opt, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`panel relative flex flex-col gap-4 p-5 sm:flex-row sm:items-center ${isBest ? 'ring-1 ring-success-500/30' : ''}`}
                >
                  {/* Rank */}
                  <div className="flex items-center gap-4 sm:w-48">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-lg font-mono text-sm font-bold ${isBest ? 'bg-success-500/15 text-success-300 ring-1 ring-success-500/30' : 'bg-white/[0.04] text-slate-400'}`}>
                  ...
                  <div className="grid flex-1 grid-cols-2 gap-3 sm:grid-cols-4">
                    <Metric label="Total cost" value={fmtInr(opt.total_cost)} tone={isBest ? 'text-success-400' : 'text-white'} />
                    <Metric label="Risk" value={opt.risk} tone={tone.text} />
                    <Metric label="Confidence" value={`${opt.confidence.toFixed(1)}%`} tone="text-accent-300" />
                    <Metric
                      label="Savings"
