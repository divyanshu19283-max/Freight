...
export function ModelIntelligence() {
  ...
  const availableHorizons = useMemo(() => HORIZONS.filter((h) => runs.some((r) => r.horizon === h)), [runs]);
  const activeHorizon = availableHorizons.includes(horizon) ? horizon : (availableHorizons[0] ?? horizon);
  const [selectedMetric, setMetric] = useState<'mae' | 'rmse' | 'mape' | 'r2'>('mae');

  const forHorizon = useMemo(() => runs.filter((r) => r.horizon === activeHorizon), [runs, activeHorizon]);
  const sorted = useMemo(() => [...forHorizon].sort((a, b) => a.mae - b.mae), [forHorizon]);
  const winner = forHorizon.find((r) => r.is_best_model) ?? sorted[0];

  const chartData = sorted.map((r) => ({ model: r.model, mae: r.mae, rmse: r.rmse, mape: r.mape, r2: r.r2 }));
    ...
    <div className="space-y-6">
      ...
      {winner && (
        <motion.div
          ...
          className="panel relative overflow-hidden p-5 ring-1 ring-accent-500/30"
        >
          <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-accent-500/10 blur-3xl" />
          <div className="relative flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-accent-500/15 ring-1 ring-accent-500/30">
              <Crown className="h-6 w-6 text-accent-300" />
            </div>
            <div>
              <div className="label-mono text-accent-300/80">Winning model · {activeHorizon}D</div>
              <div className="text-xl font-bold text-white">{winner.model}</div>
            </div>
      ...
      <div className="panel p-5">
        ...
        {q.isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 4, right: 18, left: 18, bottom: 4 }}>
              ...
              <Bar dataKey={selectedMetric} name={selectedMetric.toUpperCase()} radius={[0, 4, 4, 0]} isAnimationActive>
                {chartData.map((d) => (
      ...
      <div className="panel overflow-hidden">
        <div className="flex items-center gap-2 border-b border-white/[0.06] px-5 py-4">
          ...
          <h2 className="text-base font-semibold text-white">Model Leaderboard · {activeHorizon}D</h2>
        </div>
        {q.isLoading ? (
          <div className="p-5"><SkeletonBlock className="h-[200px] w-full" /></div>
        ) : q.isError ? (
          <div className="p-5"><InlineError error={q.error} onRetry={() => q.refetch()} /></div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3">
                        <span className={`num font-mono font-bold ${isWinner ? 'text-accent-300' : 'text-slate-500'}`}>{i + 1}</span>
                      </td>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="px-5 py-3 text-right num text-slate-300">{r.mae.toFixed(2)}</td>
                      <td className="px-5 py-3 text-right num text-slate-300">{r.rmse.toFixed(2)}</td>
                      <td className="px-5 py-3 text-right num text-slate-300">{r.mape.toFixed(2)}%</td>
                      <td className={`px-5 py-3 text-right num font-semibold ${isWinner ? 'text-accent-300' : 'text-slate-300'}`}>{r.r2.toFixed(3)}</td>
                    </motion.tr>
                  );
                })}
              </tbody>
...
function WinnerStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="text-right">
      ...
      <div className="num text-lg font-bold text-white">{value}</div>
    </div>
  );
}
