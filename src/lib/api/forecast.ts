...
function buildSeries(history: BackendHistoryResponse, forecast: BackendForecastResult): ForecastPoint[] {
...
function assertValidForecastInput(body: { origin: string; destination: string; vessel: string; horizon: number }) {
...
export async function getForecast(body: {
  ...
  const [forecast, history] = await Promise.all([
    request<BackendForecastResult>('/api/forecast', { method: 'POST', body: JSON.stringify(backendBody) }),
