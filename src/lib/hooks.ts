...
const isOffline = (e: unknown) => e instanceof ApiClientError && (e.kind === 'offline' || e.kind === 'server' || e.kind === 'unknown');
...
export function useMarketSignals(origin?: string, vesselType?: string) {
  return useQuery({ queryKey: ['market-signals', origin, vesselType], queryFn: () => getMarketSignals(origin, vesselType), retry: 0, staleTime: 60_000 });
}
