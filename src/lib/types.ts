...
export interface IntegratedDecisionResult {
  recommendation: { contract: string; voyages: number; vessel_type: string; port: string; reason: string };
  contract_options: Array<{ voyages: number; contract: string; label: string; effective_rate: number; forecast_rate: number; confidence: number; lower_bound: number; upper_bound: number; freight_cost: number; fuel_cost: number; congestion_idle_cost: number; uncertainty_cost: number; commitment_cost: number; total_cost: number; cost_per_ton: number; discount_pct: number; savings_vs_spot: number }>;
  vessel_ranking: Array<{ vessel_id: string; vessel_type: string; score: number; cargo_capacity: number; draft: number; loa: number; beam: number; daily_charter_rate: number; feasibility: BackendFeasibilityResult }>;
  selected_vessel: BackendVessel;
  voyage: BackendVoyageResult;
  idle_management: { trigger: string; current_idle_days: number; recommended_strategy: string; alternatives: Array<{ port_id: string; port_name: string; congestion_level: string; congestion_index: number; reposition_days: number; reposition_cost_usd: number; idle_days_avoided: number; feasibility: string; strategy: string }>; deadheading_note: string };
  risk_alerts: Array<{ level: string; type: string; message: string }>;
  data_provenance: { market_history: string; port_master: string; route_distance: string; live_port_feed: boolean };
}

export interface MarketSignals { freight_rate?: number; fuel_price?: number; demand_index?: number; supply_index?: number; port_congestion_index?: number; demand_supply_pressure?: number; observations: number; data_classification: string; }
